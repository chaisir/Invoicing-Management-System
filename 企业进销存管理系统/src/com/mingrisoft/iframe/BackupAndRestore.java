...
public class BackupAndRestore extends JInternalFrame {// 数据库备份与恢复内部窗体
	...
	private JPanel getJContentPane() {// 获得内容面板
		if (jContentPane == null) {// 内容面板不存在
			...
			jContentPane.setLayout(new BoxLayout(getJContentPane(), BoxLayout.Y_AXIS));// 设置内容面板的布局为垂直排列布局
	...
	private JPanel getBackupPanel() {// 获得数据库备份面板
		if (backupPanel == null) {// 数据库备份面板不存在
			...
			backupPanel.setBorder(BorderFactory.createTitledBorder(null, "\u6570\u636e\u5e93\u5907\u4efd", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD, 12), Color.blue));// 设置数据库备份面板的边框样式和字体内容
	...
	private JPanel getRestorePanel() {// 获得数据库恢复面板
		if (restorePanel == null) {// 数据库恢复面板不存在
			...
			restorePanel.setBorder(BorderFactory.createTitledBorder(null, "\u6570\u636e\u5e93\u6062\u590d", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD, 12), Color.red));// 设置数据库恢复面板的边框样式和字体内容
	...
	private JTextField getBackupTextField() {// 获得“数据库备份”文本框
		...
		backupTextField.setFont(new Font("幼圆", Font.PLAIN, 14));// 设置 “数据库备份”文本框的字体样式和字体大小
			...
			backupButton.addActionListener(new java.awt.event.ActionListener() {// “备份”按钮添加动作事件的监听
				public void actionPerformed(ActionEvent e) {
					try {
						...
						JOptionPane.showMessageDialog(BackupAndRestore.this, message);// 弹出异常信息提示框
						return;// 退出应用程序
					}
					JOptionPane.showMessageDialog(BackupAndRestore.this, "备份成功");// 弹出“备份成功”的提示框
				}
			});
		}
	...
	private JButton getBrowseButton2() {// 获得“浏览”按钮
		if (browseButton2 == null) {// “浏览”按钮不存在
			...
			browseButton2.addActionListener(new java.awt.event.ActionListener() {// 为“浏览”按钮添加动作事件的监听
				public void actionPerformed(java.awt.event.ActionEvent e) {
					JFileChooser dirChooser=new JFileChooser("./backup/");// 把“./backup/”作为路径创建文件选择器
					int option = dirChooser.showOpenDialog(BackupAndRestore.this);// 获得“打开”、“取消”的返回值
					if(option==JFileChooser.APPROVE_OPTION){// 点击“打开”按钮
						File selFile = dirChooser.getSelectedFile();// 获得文件选择器中的文件
						restoreTextField.setText(selFile.getAbsolutePath());// 设置“数据库恢复”文本框中的文本内容
					}
				}
			});
		}
	...
	private JButton getRestoreButton() {// 获得“恢复”按钮
		if (restoreButton == null) {// “恢复”按钮不存在
			...
			restoreButton.addActionListener(new java.awt.event.ActionListener() {// 为“恢复”按钮添加动作事件的监听
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					JOptionPane.showMessageDialog(BackupAndRestore.this, "恢复成功");// 弹出“恢复成功”的提示框
				}
			});
		}
